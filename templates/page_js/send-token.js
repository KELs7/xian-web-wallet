function sendToken() {
    Promise.all([
        ...
        getVariable(contract, "balances", values[0].vk).then(async balance => {
            ...
            const signed_tx = await signTransaction(transaction, unencryptedMnemonic, values[0].vk);
...
async function loadPage() {
    ...
    let tokenBalance = await execute_balance_of(document.getElementById('tokenName').innerHTML, selectedAccount.vk);
